  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'fillDoubanForm') {
      ...
      chrome.storage.local.set({ pendingAlbumData: albumData }, () => {
        ...
        chrome.tabs.create({ url: 'https://music.douban.com/new_subject' }, (tab) => {
          devLog('Tab created with ID:', tab?.id, '- Waiting for page load')
          const tabId = tab?.id
          if (tabId) {
            // Delay execution to ensure DOM is fully loaded
            setTimeout(() => {
              devLog('Initial delay complete, checking page state before filling form')
              chrome.scripting.executeScript(
                {
                  ...
                  func: function () {
                    ...
                    const titleInput = document.querySelector('input[name="p_title"]')
          ...
          chrome.scripting.executeScript(
            {
              ...
              func: function () {
                ...
                const otherInputs = {
                  ...
                  trackList: !!document.querySelector('textarea[name="p_52_other"]'),
                }
...
function fillRemainingForm() {
  ...
  chrome.storage.local.get(['pendingAlbumData'], (result) => {
    ...
    if (albumData) {
      ...
      devLog(`Form filling completed: found ${fieldsFound}/${fieldsTotal} fields`)
